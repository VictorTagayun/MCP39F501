...
void setup()
{
  ...
  timer2.setInterval(5000L, auto_measure);
...
void read_and_chk_packet()
{
  ...
  {
    // check checksum
    for (int cntr = 0; cntr < (data_cntr); cntr++)
    {
      checksum = checksum + data_received[data_cntr];
    }
...
void display_values()
{
  ...
  // display to terminal
  terminal.print("Voltage (RMS)  : "); terminal.print(voltage_rms);
  terminal.print("Current (RMS)  : "); terminal.print(current_rms);
  terminal.print("Active Power   : "); terminal.print(active_power);
  terminal.print("Reactive Power : "); terminal.print(reactive_power);
  terminal.print("Apparent Power : "); terminal.print(apparent_power);
  terminal.print("Power factor   : "); terminal.print(power_factor);
  terminal.print("Line Freq.     : "); terminal.print(line_freq);
...
void auto_measure()
{
  char read_packet[8] = {0xA5, 0x08, 0x41, 0x00, 0x0, 0x4e, numdata, 0xff}; //read values
  char send_checksum = 0x00;
  if (rts)
  {
    digitalWrite(rts_pin, LOW);
  ...
  for (int cntr = 0 ; cntr < 7; cntr++)
  {
    send_checksum = send_checksum + read_packet[cntr];
  }
  read_packet[7] = send_checksum;
  for (int cntr = 0 ; cntr < 8; cntr++)
  {
    Serial.write(read_packet[cntr]);
    //Serial1.write(read_packet[cntr]);
